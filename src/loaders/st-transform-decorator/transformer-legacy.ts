...
const buildClassDecorator: any = template(`
  DECORATOR(CLASS_REF = INNER) || CLASS_REF;
`);

const buildClassPrototype: any = template(`
  CLASS_REF.prototype;
`);

const buildGetDescriptor: any = template(`
    Object.getOwnPropertyDescriptor(TARGET, PROPERTY);
`);

const buildGetObjectInitializer: any = template(`
 ...
 */
function applyEnsureOrdering(path: any) {
    // TODO: This should probably also hoist computed properties.
    const decorators = (path.isClass() ? [path].concat(path.get('body.body')) : path.get('properties')).reduce(
        (acc: any, prop: any) => acc.concat(prop.node.decorators || []),
        [],
    );

    const identDecorators = decorators.filter((decorator: any) => !t.isIdentifier(decorator.expression));
 ...
 */
function applyClassDecorators(classPath: any) {
    ...
    return decorators
        .map((dec: any) => dec.expression)
        .reverse()
        .reduce(function(acc: any, decorator: any) {
 ...
 * with the proper decorated behavior.
 */
function applyMethodDecorators(path: any, state: any) {
    if (!hasMethodDecorators(path.node.body.body)) return;
    return applyTargetDecorators(path, state, path.node.body.body);
}

function hasMethodDecorators(body: any) {
    return body.some((node: any) => node.decorators && node.decorators.length);
}
 ...
 * with the proper decorated behavior.
 */
function applyObjectDecorators(path: any, state: any) {
 ...
 */
function applyTargetDecorators(path: any, state: any, decoratedProps: any) {
    const name = path.scope.generateDeclaredUidIdentifier(path.isClass() ? 'class' : 'obj');

    const exprs = decoratedProps.reduce(function(acc: any, node: any) {
        ...
        if (t.isClassProperty(node, { static: false })) {
            ...
            node.value = t.callExpression(state.addHelper('initializerWarningHelper'), [
                descriptor,
                t.thisExpression(),
            ]);
            ...
            acc = acc.concat([
                t.assignmentExpression(
                    ...
                    t.callExpression(state.addHelper('applyDecoratedDescriptor'), [
                        ...
                        t.arrayExpression(decorators.map((dec: any) => t.cloneNode(dec.expression))),
                        t.objectExpression([
...
function decoratedClassToExpression(opts: any) {
    const { node, scope } = opts;

    if (!hasClassDecorators(node) && !hasMethodDecorators(node.body.body)) {
        return;
    }

    const ref = node.id ? t.cloneNode(node.id) : scope.generateUidIdentifier('class');

    return t.variableDeclaration('let', [t.variableDeclarator(ref, t.toExpression(node))]);
}

export const legacyVisitor = {
        ...
        if (replacement) {
            const [varDeclPath] = path.replaceWithMultiple([
                ...
                t.exportNamedDeclaration(null, [
                    t.exportSpecifier(t.cloneNode(replacement.declarations[0].id as any), t.identifier('default')),
                ]),
            ]);
    ...
    AssignmentExpression(path: any, state: any) {
        ...
        const propertyName = path.get('left.property').node.name || path.get('left.property').node.value;
        path.replaceWith(
            t.callExpression(state.addHelper('initializerDefineProperty'), [
                t.cloneNode(path.get('left.object').node),
                t.stringLiteral('$$__transformer_legacy__$$' + propertyName),
